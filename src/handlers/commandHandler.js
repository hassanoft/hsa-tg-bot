...
export async function loadCommands() {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (
          ...
          log.warn(
            `Commande invalide ignorée : ${category}/${file}`
          );
          continue;
        }
        ...
        log.error(
          `Échec du chargement de la commande ${category}/${file}`,
          err.message,
          err.stack
        );
      }
  ...
  log.info(
    `${commands.size} commandes chargées depuis ${categories.length} catégories.`
  );
...
export function getCommand(name) {
  ...
  return (
    commands.get(name) ||
    commands.get(aliasMap.get(name)) ||
    null
  );
}
...
export function getCategories() {
  return [
    ...new Set(
      getAllCommands().map((c) => c.category)
    ),
  ];
}

/**
 * Exécute une commande à partir d'un contexte
 * déjà construit par messageHandler.js.
 */
export async function dispatchCommand(ctx) {
  ...
  if (!command) {
    await ctx.reply(
      unknownCommandMessage(config.prefix)
    );
    return;
  }
  ...
  // ---------------------------------------------------
  //
  // Le propriétaire est exempté.
  //
  // /start reste accessible afin que l'utilisateur
  // puisse rejoindre le canal.
  //
  // Toutes les autres commandes nécessitent
  // l'abonnement au canal.
  //
  if (
    config.forceJoin?.enabled &&
    !ctx.isOwner &&
    command.name !== 'start'
  ) {
  ...
  if (
    db.getSetting('maintenance', false) &&
    !ctx.isOwner
  ) {
  ...
  if (db.isBanned(ctx.senderId)) {
    await ctx.reply(
      errorMessage(
        "Vous n'êtes pas autorisé à utiliser H$Λ BOT."
      )
    );
    return;
  }
  ...
  if (isRateLimited(ctx.senderId)) {
    await ctx.reply(
      errorMessage(
        'Trop de commandes envoyées. Merci de patienter quelques secondes.'
      )
    );
    return;
  }
  ...
  const permission =
    checkCommandPermissions(command, ctx);
  ...
  if (!permission.allowed) {
    const messages = {
      ...
      private: errorMessage(
        'Cette commande fonctionne uniquement en message privé.'
      ),
      'bot-admin':
        botAdminRequiredMessage(),
    };
  ...
  try {
    ...
    log.error(
      `Erreur lors de l'exécution de /${command.name}`,
      err.message,
      err.stack
    );
